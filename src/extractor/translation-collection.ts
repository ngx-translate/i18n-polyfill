// source: https://github.com/biesbjerg/ngx-translate-extract/blob/master/src/utils/translation.collection.ts
...
export class TranslationCollection {
  ...
  public intersect(collection: TranslationCollection): TranslationCollection {
    const values: TranslationType = {};
    this.filter(key => collection.has(key)).forEach((key: string, val: string) => {
      values[key] = val;
    });
